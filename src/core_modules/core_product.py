...
update_new = (
    "UPDATE product SET product_name = %s, quantity= %s, price = %s WHERE ID = %s"
)
...
def fetch_products(conn):
    ...
    for raw in product:
        products.append(
            {"product_name": raw[1], "quantity": raw[2], "price": raw[3], "id": raw[0]}
        )
    return products
...
def view_products(state):
    ...
    print(
        tabulate.tabulate(
            print_products, headers="keys", tablefmt="fancy_grid", showindex=True
        )
    )
    return state
...
def update_products(state, conn):
    view_products(state)
    idx = pyip.inputNum(
        "please select a product to update: ", min=0, max=len(state["products"]) - 1
    )

    for key in state["products"][idx].keys():

        if key == "price":
            update = pyip.inputFloat(
